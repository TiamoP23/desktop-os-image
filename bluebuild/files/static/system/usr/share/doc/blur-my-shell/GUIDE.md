...
The library can be installed directly on this image by running:

```
/usr/local/bin/rounded_blur_build.sh -i
```

If you prefer the upstream helper, you can still run:
...
The library can be uninstalled directly on this image by running:

```
/usr/local/bin/rounded_blur_build.sh -u
```
